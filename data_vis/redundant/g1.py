...
disp_C = load_grid("outputs/disp_C_g_0011.txt")
true_g = load_grid("outputs/true_g_0011.txt")

# Determine common color scale limits
vmin = min(np.nanmin(disp_C), np.nanmin(true_g))
vmax = max(np.nanmax(disp_C), np.nanmax(true_g))
...
target_shape = (
    max(disp_C.shape[0], true_g.shape[0]),  # Max rows
    max(disp_C.shape[1], true_g.shape[1])   # Max cols
)

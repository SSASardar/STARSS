...
def plot_dual_sums_histogram(file_path, save_path=None, bins=30):
    """
    Plot overlaid histograms of sums values from columns 8 and 9.
    
    Parameters:
    -----------
    file_path : str
        Path to the sums results file
    ...
    # Extract sums values (8th column = index 7, 9th column = index 8)
    if data.shape[1] >= 9:
        sums1 = data[:, 7]   # 8th column
        sums2 = data[:, 8]   # 9th column
        print(f"Extracted {len(sums1)} sums values from column 8")
        print(f"Extracted {len(sums2)} sums values from column 9")
    else:
    ...
    all_values = np.concatenate([sums1, sums2])
    ...
    ax.hist(sums1, bins=n_bins, alpha=0.6, color=stylesheet.COLORS['blue'], 
            edgecolor='black', linewidth=0.5, label='Strategy 1 (Column 8)', 
            density=True)
    ax.hist(sums2, bins=n_bins, alpha=0.6, color=stylesheet.COLORS['orange'], 
    ...
    ax.set_xlabel('Sums Value', fontsize=13, fontweight='bold')
    ax.set_ylabel('Density', fontsize=13, fontweight='bold')
    ax.set_title('Sums Distribution: Two Scanning Strategies', fontsize=14, fontweight='bold')
    ...
    print("-"*64)
    print(f"{'Count':<20} {len(sums1):<22} {len(sums2):<22}")
    ...
    print(f"{'25th %ile':<20} {np.percentile(sums1, 25):<22.6f} {np.percentile(sums2, 25):<22.6f}")
    print(f"{'75th %ile':<20} {np.percentile(sums1, 75):<22.6f} {np.percentile(sums2, 75):<22.6f}")
    print(f"{'IQR':<20} {np.percentile(sums1, 75)-np.percentile(sums1, 25):<22.6f} {np.percentile(sums2, 75)-np.percentile(sums2, 25):<22.6f}")
    print("-"*64)
...
def plot_sums_scatter(file_path, save_path=None, alpha=0.6, add_diagonal=True):
    """
    Create a scatter plot comparing sums values from columns 8 and 9.
    ...
    # Extract sums values (8th column = index 7, 9th column = index 8)
    if data.shape[1] >= 9:
        sums1 = data[:, 7]   # 8th column (Strategy 1)
        sums2 = data[:, 8]   # 9th column (Strategy 2)
        print(f"Extracted {len(sums1)} sums values from column 8")
        print(f"Extracted {len(sums2)} sums values from column 9")
    else:
        print(f"Error: File has only {data.shape[1]} columns, expected at least 9")
    ...
    scatter = ax.scatter(sums1, sums2, alpha=alpha, s=30, 
                        color=stylesheet.COLORS['blue'],
                        edgecolor='white', linewidth=0.5)
    
    # Add diagonal line (y=x) for reference
    if add_diagonal:
        min_val = min(np.min(sums1), np.min(sums2))
        max_val = max(np.max(sums1), np.max(sums2))
        # Add some padding
    ...
    # Add labels and title
    ax.set_xlabel('Sums Strategy 1 (Column 8)', fontsize=13, fontweight='bold')
    ax.set_ylabel('Sums Strategy 2 (Column 9)', fontsize=13, fontweight='bold')
    ax.set_title('Comparison of Sums Values: Strategy 1 vs Strategy 2', 
    ...
    # Add statistics box
    correlation = np.corrcoef(sums1, sums2)[0, 1]
    diff_mean = np.mean(sums2 - sums1)
    diff_std = np.std(sums2 - sums1)
    
    stats_text = (
        f'Correlation: {correlation:.6f}\n'
        f'Mean Diff (S2-S1): {diff_mean:.6f}\n'
        f'Std Diff (S2-S1): {diff_std:.6f}\n'
        f'Min Diff: {np.min(sums2 - sums1):.6f}\n'
        f'Max Diff: {np.max(sums2 - sums1):.6f}'
    )
    ...
    print(f"  Min: {np.min(sums2 - sums1):.6f}")
    print(f"  Max: {np.max(sums2 - sums1):.6f}")
    print(f"  25th percentile: {np.percentile(sums2 - sums1, 25):.6f}")
    print(f"  75th percentile: {np.percentile(sums2 - sums1, 75):.6f}")
    
    return fig


def plot_dual_sums_comparison(file_path, save_dir=None):
    """
    ...
    print("="*80)
    if save_dir:
        hist_path = f"{save_dir}/dual_sums_histogram.png"
    else:
        hist_path = None
    plot_dual_sums_histogram(file_path, save_path=hist_path)
    ...
    print("="*80)
    if save_dir:
        scatter_path = f"{save_dir}/sums_scatter.png"
    else:
        scatter_path = None
    plot_sums_scatter(file_path, save_path=scatter_path)
...
# 1. Histogram only
plot_dual_sums_histogram("batch_test_20260827_113654/results_sums.txt")

# 2. Scatter plot only
plot_sums_scatter("batch_test_20260827_113654/results_sums.txt")

# 3. Scatter plot with custom transparency and save
# plot_sums_scatter(
#     "batch_test_20260827_102829/sums_results.txt",
#     save_path="sums_scatter.png",
#     alpha=0.4
# )

...
ax.plot(time_C, total_true_mm2_C, 
        marker='s', 
        color=stylesheet.COLORS['orange'], 
...
ax.axvspan(0, t1, alpha=0.15, color=stylesheet.COLORS['gray'], label='_nolegend_')
ax.text(0.05 * t1, ymax - 0.05 * y_range, 'UNINITIATED', 
        ha='left', va='top', fontsize=9, fontweight='bold', 
        color=stylesheet.COLORS['gray'], alpha=0.7)

# GROWTH region (60 to 120 min) - green
ax.axvspan(t1, t2, alpha=0.15, color=stylesheet.COLORS['green'], label='_nolegend_')
ax.text(t1 + 0.05 * (t2 - t1), ymax - 0.05 * y_range, 'GROWTH', 
        ha='left', va='top', fontsize=9, fontweight='bold', 
        color=stylesheet.COLORS['green'], alpha=0.7)

# MATURE region (120 to 170 min) - blue
ax.axvspan(t2, t3, alpha=0.15, color=stylesheet.COLORS['blue'], label='_nolegend_')
ax.text(t2 + 0.05 * (t3 - t2), ymax - 0.05 * y_range, 'MATURE', 
        ha='left', va='top', fontsize=9, fontweight='bold', 
        color=stylesheet.COLORS['blue'], alpha=0.7)

# DECAY region (170 to 230 min) - red
ax.axvspan(t3, t5, alpha=0.15, color=stylesheet.COLORS['red'], label='_nolegend_')
ax.text(t3 + 0.05 * (t5 - t3), ymax - 0.05 * y_range, 'DECAY', 
        ha='left', va='top', fontsize=9, fontweight='bold', 
        color=stylesheet.COLORS['red'], alpha=0.7)

# POST-DECAY region (after 230 min) - same as UNINITIATED (gray)
ax.axvspan(t5, xmax, alpha=0.15, color=stylesheet.COLORS['gray'], label='_nolegend_')
...
ax.set_title("Rainfall accumulation from cell", fontsize=11)
...
plt.tight_layout()
plt.savefig("stats_comparison.pdf", bbox_inches='tight')
plt.savefig("stats_comparison.png", dpi=300, bbox_inches='tight')
plt.show()

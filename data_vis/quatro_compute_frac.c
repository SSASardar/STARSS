...
typedef struct {
    ...
    double x_stats_discrepancy;      // (True - Measured) / Sum(True) for X-band stats
    double c_stats_discrepancy;      // (True - Measured) / Sum(True) for C-band stats
    double combi_stats_discrepancy;  // (True - Measured) / Sum(True) for combi stats
    double ad_stats_discrepancy;     // (True - Measured) / Sum(True) for AD stats
...
// Read stats file and compute normalized discrepancy
// Normalized discrepancy = (Sum(True) - Sum(Measured)) / Sum(True)
int read_stats_file(const char* filename, double* discrepancy, int* scan_count) {
    ...
    // Compute normalized discrepancy: (True - Measured) / True
    // This gives the relative error, e.g., 0.1 means 10% underestimation
    if (fabs(sum_true) < 1e-12) {
        // Avoid division by zero; if true sum is zero, discrepancy is undefined
        *discrepancy = NAN;
    } else {
        *discrepancy = (sum_true - sum_meas) / sum_true;
    }
    *scan_count = count;
...
int process_batch_folder(const char* folder_path) {
    ...
    printf("=========================================\n");
    printf("  NORMALIZED RAINFALL ACCUMULATION DISCREPANCY\n");
    printf("  (True - Measured) / True\n");
    printf("=========================================\n");
    ...
    snprintf(output_file, sizeof(output_file), "%s/results_fracs.txt", folder_path);
    ...
    fprintf(out, "# Normalized Rainfall Accumulation Discrepancy Results\n");
    fprintf(out, "# Normalized Discrepancy = (Sum(True) - Sum(Measured)) / Sum(True)\n");
    ...
    fprintf(out, "#   X_Stats_Discrepancy - Normalized discrepancy for X-band stats\n");
    fprintf(out, "#   C_Stats_Discrepancy - Normalized discrepancy for C-band stats\n");
    fprintf(out, "#   Combi_Stats_Discrepancy - Normalized discrepancy for combi stats\n");
    fprintf(out, "#   AD_Stats_Discrepancy - Normalized discrepancy for AD stats\n");
    ...
    fprintf(out, "#   Example: 0.10 means 10%% underestimation, -0.05 means 5%% overestimation\n");
    fprintf(out, "#\n");
    ...
    printf("\n");
    printf("Summary (Normalized Discrepancy = (True - Measured) / True):\n");
    ...
    if (x_stats_valid > 0) {
        printf("  X Stats Normalized Discrepancy:\n");
    ...
    if (c_stats_valid > 0) {
        printf("\n  C Stats Normalized Discrepancy:\n");
        printf("    Valid values: %d\n", c_stats_valid);
    ...
    if (combi_stats_valid > 0) {
        printf("\n  Combi Stats Normalized Discrepancy:\n");
        printf("    Valid values: %d\n", combi_stats_valid);
    ...
    if (ad_stats_valid > 0) {
        printf("\n  AD Stats Normalized Discrepancy:\n");
        printf("    Valid values: %d\n", ad_stats_valid);

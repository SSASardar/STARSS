...
ax.set_xlabel('Discrepancy in rainfall accumulation [mm]')
ax.set_ylabel('Count')
ax.set_title('Distribution of rainfall accumulation discrepancy', pad=10)
...
plt.tight_layout()
plt.savefig('figures/cs_discrepancy_histogram.png',
            facecolor='white', transparent=False)
plt.savefig('figures/cs_discrepancy_histogram.pdf',
            facecolor='white', transparent=False)
print("\nFigure saved: figures/cs_discrepancy_histogram.png and .pdf")
plt.show()

# ============================================================
# 4. SCATTER — combined vs C, adaptive vs C
# ============================================================

fig2, axes = plt.subplots(1, 2, figsize=(11, 5.5),
                          sharex=True, sharey=True)

panels = [
    (axes[0], z_c, z_combi, col_combi,
     'Combined vs C-band PPI',
     'C-band PPI + X-band PPI'),
    (axes[1], z_c, z_ad, col_ad,
     'Adaptive vs C-band PPI',
     'C-band PPI + X-band RHI'),
]

for ax2, z_ref, z_other, colour, title, ylabel in panels:
    # 1:1 reference line
    lo = min(z_ref.min(), z_other.min())
    hi = max(z_ref.max(), z_other.max())
    ...
    # Zero lines
    ax2.axhline(0, color=stylesheet.COLORS['gray'],
                linewidth=0.6, linestyle=':',
                alpha=0.6, zorder=0)
    ax2.axvline(0, color=stylesheet.COLORS['gray'],
                linewidth=0.6, linestyle=':',
                alpha=0.6, zorder=0)

    # Scatter
    ax2.scatter(z_ref, z_other,
                s=14, alpha=0.35,
                color=colour, edgecolors='none',
                zorder=2, label=ylabel)

    ax2.set_xlabel('C-band PPI discrepancy [mm]')
    ax2.set_ylabel('Comparison discrepancy [mm]')
    ...
    # Equal aspect so the 1:1 line really is at 45 degrees
    ax2.set_aspect('equal', adjustable='box')

plt.tight_layout()
plt.savefig('figures/cs_discrepancy_scatter.png',
            facecolor='white', transparent=False)
plt.savefig('figures/cs_discrepancy_scatter.pdf',
            facecolor='white', transparent=False)
print("Figure saved: figures/cs_discrepancy_scatter.png and .pdf")
plt.show()

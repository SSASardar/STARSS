...
HashNode* find_or_create(int r, int m, int c, int k, int y, int a) {
    ...
    node->y = y;
    node->a = a;
    node->mse_regular = NAN;
    node->mse_adaptive = NAN;
    node->has_regular = 0;
    node->has_adaptive = 0;
...
// Read stats file and compute average MSE (skip header row)
double compute_avg_mse_from_file(const char* filename) {
...
int process_batch_folder(const char* folder_path) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strstr(entry->d_name, "stats_r_") != NULL && 
            ...
            double avg_mse = compute_avg_mse_from_file(filepath);
            if (!isnan(avg_mse)) {
                HashNode* node = find_or_create(r, m, c, k, y, a);
                if (node) {
                    node->mse_regular = avg_mse;
                    node->has_regular = 1;
                    regular_count++;
                }
            } else {
    ...
    snprintf(output_file, sizeof(output_file), "%s/mse_results.txt", folder_path);
    ...
    fprintf(out, "# Average MSE Results\n");
    fprintf(out, "# Generated: %s %s\n", __DATE__, __TIME__);
    fprintf(out, "# Format: r m c k y a regular_MSE adaptive_MSE\n");
    ...
    for (int i = 0; i < MAX_HASH_TABLE; i++) {
        ...
        while (node) {
            ...
            entries[entry_count].mse_regular = node->mse_regular;
            entries[entry_count].mse_adaptive = node->mse_adaptive;
    ...
    for (int i = 0; i < entry_count; i++) {
        ...
        if (entries[i].has_regular) {
            fprintf(out, "%.10f ", entries[i].mse_regular);
            sum_regular += entries[i].mse_regular;
            regular_valid++;
        } else {
            fprintf(out, "NA ");
        }
        ...
        if (entries[i].has_adaptive) {
            fprintf(out, "%.10f\n", entries[i].mse_adaptive);
            sum_adaptive += entries[i].mse_adaptive;
            adaptive_valid++;
    ...
    printf("\n");
    if (regular_valid > 0) {
        printf("  Regular MSE mean: %.10f\n", sum_regular / regular_valid);
    }
    if (adaptive_valid > 0) {
        printf("  Adaptive MSE mean: %.10f\n", sum_adaptive / adaptive_valid);
    }
    printf("=========================================\n");

...
#    Theta, R, data0, shading='auto', cmap='viridis',
#    vmin=radar_vmin, vmax=radar_vmax
#)
#fig.colorbar(radar_pcol, ax=ax_radar, orientation="vertical", label="Reflectivity")
ax_radar.set_ylim(0, meta0.get('radar.maximum_range', r[-1]))
#ax_radar.set_title("Radar Scan (scan.index=1)")
...
#        ax_radar.set_title(f"Radar Scan (scan.index=1) - {timestamp_min} min")
        # Only show the max range label
        max_range = r[-1]
        ax_radar.set_yticks([max_range])               # place tick only at max range
        ax_radar.set_yticklabels([f"{max_range:.0f} km"])  # label it


    return [im_disp, im_true, vpr_line, vpr_scatter, time_line,
            cross_disp, cross_true, radar_pcol] + profile_lines

...
typedef struct {
    ...
    int m;  // mature phase end
    int c;  // cloud base height
    int k;  // core ratio (multiplied by 100)
    int y;  // y-distance
    int a;  // apparent motion
    double emd;
} TestResult;
...
int extract_parameters(const char* filename, TestResult* result) {
    ...
    // Pattern: stats_r_(\d+)_m_(\d+)_c_(\d+)_k_(\d+)_y_(\d+)_a_(\d+)\.txt
...
int process_batch_folder(const char* folder_path) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (norm_measured && norm_true && norm_count_meas == norm_count_true) {
            ...
            printf("    Parameters: r=%d m=%d c=%d k=%d y=%d a=%d\n", 
                   result.r, result.m, result.c, result.k, result.y, result.a);
            printf("    Scans: %d\n", scan_count);
            printf("    EMD: %.10f\n\n", result.emd);
    ...
    fprintf(out, "# Format: r m c k y a EMD\n");
    ...
    for (int i = 0; i < result_count; i++) {
        fprintf(out, "%d %d %d %d %d %d %.10f\n",
                results[i].r, results[i].m, results[i].c,
                results[i].k, results[i].y, results[i].a,
                results[i].emd);
